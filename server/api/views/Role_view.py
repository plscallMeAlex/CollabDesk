...
class RoleViewSet(ModelViewSet):
    ...
    def all_roles_and_users(self, request):
        ...
        roles_and_users = []
        for role in roles:
            users = User.objects.filter(id__in=GuildMembership.objects.filter(role=role).values_list("user", flat=True))
            roles_and_users.append({
                "role": RoleSerializer(role).data,
                "users": UserSerializer(User.objects.filter(id__in=users), many=True).data
            })
